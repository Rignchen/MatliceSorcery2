## function sorcery:action/aura/calcul/15k-plus/100k-plus/500k-plus

scoreboard players set @s sorcery.mana_aura 14

attribute @s generic.max_health modifier remove 53da1883-cb6d-4726-937e-7805dabb834a
execute if score #aura sorcery.settings matches 1 run attribute @s generic.max_health modifier add 53da1883-cb6d-4726-937e-7805dabb834a "sorcery.aura" 40 add

execute unless score @s sorcery.last_aura = @s sorcery.mana_aura run scoreboard players remove @s sorcery.max_cooldown 5
...
execute if score @s sorcery.last_aura matches ..12 run scoreboard players remove @s sorcery.max_cooldown 5
execute if score @s sorcery.last_aura matches ..13 run scoreboard players remove @s sorcery.max_cooldown 5
